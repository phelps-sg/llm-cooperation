...
results.clean <- results %>%
  convert_as_factor(
    Experiment, Participant_group, Partner_condition, t, Model, Participant_id, Participant_prompt_index,
...
interaction.plots <- function(participant.group, legend) {
  ...
  p <- ggplot(combined) +
    ...
    labs(
      title = sprintf("Group: %s", participant.group),
...
estimate_glmm <- function() {
  ...
  MCMCglmm(
    fixed = Num_cooperates ~ Participant_group + Partner_condition + t + Model + Model:Partner_condition:Participant_group,
